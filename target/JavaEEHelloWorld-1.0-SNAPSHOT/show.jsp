...
<%
    ...
    PolynomialBuilder polynomial = (PolynomialBuilder) request.getAttribute("current");
    int exponent = Integer.parseInt((String)request.getAttribute("exponent"));

    request.setAttribute("exponent", String.valueOf(exponent));
%>
<div class="session">
    <div class="<%=picture%>">
...
<style type="text/css">
    ...
    <form action="polynomial" class="log-in" autocomplete="off" method="get">
        <h4>Término <span>AGREGADO</span></h4>
        <p>Continue agregando o finalice para calcular</p>
        <%
            if (error == null) {
        %>
        <div class="floating-label">
            <input type="text" name="original" id="original"
                   autocomplete="off" disabled="on"
                   value="<%=polynomial%>"
            >
            <label for="original">Polinomio actual:</label>
        </div>
        <%
            }
        %>
